...
int main(int argc, char *argv[])
{
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp master
        {
            #pragma omp target enter data map(to: vp[0:size_u[0] * size_u[1]]) device(0)
            printf("Sent vp to GPU!\n");
            #pragma omp target enter data map(to: ut0[0:gpu_data_domain+halo]) device(0)
            printf("Sent ut0 to GPU!\n");
            #pragma omp target enter data map(to: ut1[0:gpu_data_domain+halo]) device(0)
            printf("Sent ut1 to GPU!\n");
            #pragma omp target enter data map(to: ut2[0:gpu_data_domain+halo]) device(0)
        ...
        for (int time = time_m; time <= time_M; time++){
            ...
            {
                // GPU working
                #pragma omp target teams distribute parallel for collapse(2) nowait device(0)
		...
		//update from GPU
            ...
            {
		#pragma omp taskwait
                #pragma omp target update from(ut1[gpu_data_domain-halo:halo]) device(0)
                #pragma omp target update to(ut1[gpu_data_domain:halo]) device(0)
                aux = ut1;
                ut1 = ut2;
                ut2 = ut0;
                ut0 = aux;
        ...
        #pragma omp master
        {
            #pragma omp target update from(ut0[0:gpu_data_domain]) device(0)
            #pragma omp target update from(ut1[0:gpu_data_domain]) device(0)
            #pragma omp target update from(ut2[0:gpu_data_domain]) device(0)
            #pragma omp target exit data map(release: vp[0:size_u[0] * size_u[1]]) device(0)
            #pragma omp target exit data map(release: ut0[0:gpu_data_domain+halo]) device(0)
            #pragma omp target exit data map(release: ut1[0:gpu_data_domain+halo]) device(0)
            #pragma omp target exit data map(release: ut2[0:gpu_data_domain+halo]) device(0)
        }
